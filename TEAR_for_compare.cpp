...
void oursWithTime() {
	...
	for (int t = 0; t < uvDemandPairs.size(); t++) {
		...
		int tempcost=openCost+closeCost;
		...
		if (i == 0) {
			...
			for (int j = 0; j < pathAll[i].size(); j++) {
				if (pathAll[i][j].size() != 0) {
					...
					numpath++;
					tempath+=pathAll[i][j].size();
				}
			}
			pathlen+=tempath/numpath;
			cout << "change: " << change << endl;
			cout << "pathlen: " << tempath<< endl;
			cout << "numlen: " << numpath<< endl;
	...
	cout << "total_pathlen:  " << pathlen/10 << endl;
...
void spWithTime() {
	...
	cout << "total change: " << totalChange << endl;
	cout << "total_pathlen:  " << pathlen/288 << endl;
	//outputFile << "total change: " << totalChange << endl;
}
...
void G2014WithTime() {
	...
	for (int t = 0; t < uvDemandPairs.size(); t++) {
		...
		openCost=openCost/2;
		closeCost=closeCost/2;
		int tempcost=closeCost+openCost;
...
int main()
{
	...
	{	
		resultdir="big_topo2/";
		demandDirectory = resultdir+"demand/";
		nFile = 288;
